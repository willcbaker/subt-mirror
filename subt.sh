...
do_install() {
	cat >&2 <<-'EOF_INIT'
	=======================================================
	SubT Simulation Installation Script
	=======================================================

	This script is installing the latest stable version of
	the SubT Simulator available.
	...
	case "$lsb_dist" in
		...
		ubuntu)
	        ...
	        if [ "bionic" != "$dist_version" ]; then
				echo " * Ubuntu bionic (18.04) is required."
				exit 0
			fi
			...
			$sh_c "echo deb http://packages.osrfoundation.org/gazebo/ubuntu-stable $dist_version main > /etc/apt/sources.list.d/gazebo-stable.list"
			$sh_c "echo deb http://packages.osrfoundation.org/gazebo/ubuntu-prerelease $dist_version main > /etc/apt/sources.list.d/gazebo-prerelease.list"
		...
		fedora)
			echo " * Fedora is not supported."
			exit 0
			;;
		gentoo)
			echo " * Gentoo is not supported."
			exit 0
			;;
		osX)
	...
	installer script. Please visit the following URL for more detailed
	installation instructions:

      https://bitbucket.org/osrf/subt/wiki/tutorials
